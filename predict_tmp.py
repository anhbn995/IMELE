            ...
            def process(coordinates):
                ...
                if np.count_nonzero(image_detect) > 0:
                    if len(np.unique(image_detect)) <= 2:
                        ...
                        # kwargs = meta.copy()
                        # window=Window(start_x, start_y, shape[1], shape[0])
                        # kwargs.update({
                        #             'height': window.height,
                        #             'width': window.width,
                        #             'transform': rasterio.windows.transform(window, raster.transform)})
                        # with rasterio.open(f'/home/skm/SKM16/3D/3D/xoa/tmp/cropped_{start_x}_{start_y}.tif', 'w', **kwargs) as dst:
                        #     dst.write(y_pred)
                        """Khong over lap"""
                        ...
                        a = int((y_pred.shape[0] - stride_size)/2)
                        size_pad = 10
                        b = y_pred.shape[0] - a + size_pad
                        y_pred = np.array([y_pred[a:b, a:b]])
                        kwargs = meta.copy()
                        window=Window(start_x, start_y, y_pred.shape[2], y_pred.shape[1])
                        print(a,b, y_pred.shape[2], y_pred.shape[1])
                        kwargs.update({
                                    'height': window.height,
                                    'width': window.width,
                                    'transform': rasterio.windows.transform(window, raster.transform)})
                        with rasterio.open(f'/home/skm/SKM16/3D/3D/xoa/tmp2/cropped_{start_x}_{start_y}.tif', 'w', **kwargs) as dst:
                            dst.write(y_pred)
                        """Over lap"""
            #             a = int((y_pred.shape[0] - stride_size)/2)
            #             b = y_pred.shape[0] - a
...
if __name__=="__main__":
    ...
    fp_out_dir = os.path.join(dir_out, f'RS_{crop_size}_{fn_model}_{x}.tif')
